...
import StakersUI from "./views/StakersUI";
...
function App(props) {
  ...
  const createStakerEvents = useEventListener(readContracts, "StakerFactory", "CreateStaker", localProvider, 1);
  console.log("📟 create staker events:", createStakerEvents);

  const [stakerContracts, setStakerContracts] = useState([]);
  useEffect(() => {
    if (stakerContracts.length !== createStakerEvents.length) {
      setStakerContracts(
        createStakerEvents
          .map(event => ({
            address: event.args.stakerAddress,
            name: event.args.name,
            time: new Date(event.args.timestamp.toNumber() * 1000),
            creator: event.args.creator,
            destinationContractAddress: event.args.destinationAddress,
          }))
          .reverse(),
      );
    }
  ...
  useEffect(() => {
    const load = async () => {
      const Staker = await loadNonDeployedContractAbi("Staker");
      const ExampleExternalContract = await loadNonDeployedContractAbi("ExampleExternalContract");
      setInjectableAbis({ Staker, ExampleExternalContract });
    };
    load();
  }, [setInjectableAbis]);
          ...
          <Route exact path="/">
            <div className="AppCenteredCol">
              <StakersUI
                ...
                stakerContracts={stakerContracts}
